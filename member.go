...
func (c *Client) CreateMember(data *CreateMember, listID string) (*Member, error) {
	...
	response, err := c.post(slashJoin(LISTS_URL, listID, MEMBERS_URL), nil, data)
	if err != nil {
...
func (c *Client) GetMembers(listID string, params ...Parameters) ([]*Member, error) {
	...
	response, err := c.get(slashJoin(LISTS_URL, listID, MEMBERS_URL), p)
	if err != nil {
		log.WithFields(log.Fields{
			"listID": listID,
			"error":  err.Error(),
...
func (c *Client) GetMember(id string, listID string) (*Member, error) {
	response, err := c.get(slashJoin(LISTS_URL, listID, MEMBERS_URL, id), nil)
	if err != nil {
		log.WithFields(log.Fields{
			"listID":   listID,
			"memberID": id,
...
func (m *Member) Delete() error {
	...
	err := m.client.delete(slashJoin(LISTS_URL, m.ListID, MEMBERS_URL, m.ID))
	if err != nil {
		log.WithFields(log.Fields{
			"listID":   m.ListID,
			"memberID": m.ID,
...
func (m *Member) Update(data *UpdateMember) (*Member, error) {
	...
	response, err := m.client.put(slashJoin(LISTS_URL, m.ListID, MEMBERS_URL, m.ID), nil, data)
	if err != nil {
		log.WithFields(log.Fields{
			"listID":   m.ListID,
			"memberID": m.ID,
